...
CreateConVar("randomat_whatitslike2_given_items_count", "2", {FCVAR_ARCHIVE, FCVAR_NOTIFY}, "How many most bought items to give out", 1, 10)
...
local function GetDescription()
    ...
    if count == 1 then
        ...
        return "Everyone gets someone's playermodel and their " .. GetConVar("randomat_favourites2_given_items_count"):GetInt() .. " most bought items!"
    end
end

util.AddNetworkString("WhatItsLike2RandomatHideNames")
util.AddNetworkString("WhatItsLike2RandomatEnd")
EVENT.Title = ""
EVENT.id = "whatitslike2"
EVENT.Description = GetDescription()
EVENT.AltTitle = "What it's like to actually be ..."

CreateConVar("randomat_whatitslike2_disguise", 0, {FCVAR_NOTIFY, FCVAR_ARCHIVE}, "Hide player names")
...
function EVENT:Begin()
    ...
    local itemCount = math.min(GetConVar("randomat_whatitslike2_given_items_count"):GetInt(), table.Count(equipmentStats))
    ...
    if CR_VERSION and GetConVar("randomat_whatitslike_disguise"):GetBool() then
        net.Start("WhatItsLike2RandomatHideNames")
        net.Broadcast()
    end
...
function EVENT:End()
    ...
    if CR_VERSION then
        net.Start("WhatItsLike2RandomatEnd")
        net.Broadcast()
    end
end
